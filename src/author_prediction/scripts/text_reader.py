"""
Sentence segmentation, normalization, and sliding-door windowing
for multi-authorship / authorship-attribution analysis of historical documents.

Design principles for this domain:
- Normalization should be LIGHT: aggressive lowercasing, punctuation stripping,
  or stemming destroys the exact signal stylometry relies on (function words,
  punctuation rhythm, capitalization habits, sentence length). We only fix
  things that are OCR/encoding noise, not authorial or period style.
- Sentence segmentation must be abbreviation-aware, since historical texts
  are full of "Mr.", "St.", "Capt.", "viz.", etc. that a naive splitter
  will treat as sentence boundaries.
- The "sliding door" (a.k.a. sliding window) technique moves a fixed-size
  window of N sentences across the document in steps of size S < N, so
  consecutive windows overlap. A stylometric feature vector is computed per
  window; abrupt shifts in that feature vector as the window slides are
  the signal used to infer a change of author (this is the core trick
  behind style-change / multi-authorship detection, e.g. Koppel & Schler-
  style "unmasking" pipelines and PAN style-breach-detection systems).
"""
...
# Common historical / OCR artifacts worth fixing because they're NOISE,
# not style: encoding variants of the same character, not variants Anthropic
# would want to treat as distinct authorial choices.
...
def normalize_text(text: str, fix_long_s: bool = True) -> str:
    """
    Light normalization: Unicode canonicalization, long-s (ſ) repair,
    ligature/quote unification, and whitespace collapse.

    Deliberately does NOT lowercase, strip punctuation, or touch spelling —
    those are candidate stylometric features, not noise, in this pipeline.
    """
    text = unicodedata.normalize("NFKC", text)
    text = text.translate(_QUOTE_MAP)

    if fix_long_s:
        # historical long-s ("ſ") -> "s", but not inside "ſt"/"ſſ" ligature
        # edge cases you may need to special-case for your corpus
        text = text.replace("ſ", "s")

    # collapse runs of whitespace/newlines from OCR line-wrapping, but
    # preserve paragraph breaks (double newline) as a single marker,
    # since paragraph structure can itself be a useful feature later.
    text = re.sub(r"[ \t]+", " ", text)
    text = re.sub(r"\n{2,}", "\n\n", text)
    text = re.sub(r"(?<!\n)\n(?!\n)", " ", text)  # single newline = soft wrap
    return text.strip()
...
# Extend this per corpus/period. Keep lowercase; matching is case-insensitive.
DEFAULT_ABBREVIATIONS = {
    "mr", "mrs", "ms", "dr", "st", "sr", "jr", "prof", "rev",
    "gen", "col", "capt", "lt", "sgt", "hon", "esq",
    "vs", "etc", "viz", "cf", "al", "no", "vol", "ch", "p", "pp",
    "i.e", "e.g", "a.m", "p.m",
}

@dataclass
class SentenceSplitter:
    """Abbreviation-aware sentence segmenter using a single regex pass
    plus a lookback check, rather than a full NLP pipeline — keeps this
    dependency-free and auditable, which matters for a research pipeline
    where you want to know exactly why a boundary was (or wasn't) placed.

    If your corpus is large/diverse enough to justify it, swap this out
    for spaCy's sentencizer or PySBD and keep the same interface.
    """
    abbreviations: set = field(default_factory=lambda: set(DEFAULT_ABBREVIATIONS))
    # sentence-ending punctuation, allowing trailing quotes/brackets
    _end_pattern: re.Pattern = field(init=False, repr=False)

    def __post_init__(self):
        self._end_pattern = re.compile(r'([.!?]+)([\'"”’\)\]]*)(\s+)(?=[A-Z0-9"\'“‘(])')

    def _is_abbreviation(self, token: str) -> bool:
        token = token.rstrip(".").lower()
        return token in self.abbreviations

    def split(self, text: str) -> List[str]:
        candidates = []
        pos = 0
        for m in self._end_pattern.finditer(text):
            boundary_end = m.end()
            preceding = text[pos:m.start(1) + len(m.group(1))]
            last_word = re.findall(r"[A-Za-z\.]+$", text[pos:m.start(1)].strip())
            if last_word and self._is_abbreviation(last_word[-1]):
                continue  # not a real sentence boundary
            candidates.append(text[pos:boundary_end].strip())
            pos = boundary_end
        tail = text[pos:].strip()
        if tail:
...
def segment_sentences(text: str, splitter: Optional[SentenceSplitter] = None) -> List[str]:
...
# Punctuation kept as separate tokens on purpose — punctuation rate/rhythm
# is a stylometric feature, not noise to discard.
_TOKEN_PATTERN = re.compile(
    r"[A-Za-z'\u2019]+(?:-[A-Za-z'\u2019]+)*"  # words, incl. hyphenated/contractions
    r"|[.,;:!?\"'()\[\]—-]"                     # punctuation as individual tokens
    r"|\d+"                                     # numbers
)
...
def tokenize(
    ...
    """
    Regex word/punctuation tokenizer. Deliberately simple and
    period-agnostic rather than a modern NLP tokenizer, since taggers
    trained on contemporary text can mishandle archaic spelling and
    punctuation conventions.

    lowercase=False by default — keep case for capitalization-sensitive
    features; pass lowercase=True when you specifically need normalized
    counts (e.g. function-word frequency).

    merge_abbreviations=True re-joins a known abbreviation word with its
    following period ("Mr" + "." -> "Mr.") so abbreviation periods don't
    inflate punctuation-rate counts, and "Mr." doesn't get split into a
    separate word token that pollutes word-frequency stats. Uses
    DEFAULT_ABBREVIATIONS unless a custom set is passed.
    """
    tokens = _TOKEN_PATTERN.findall(text)
    abbrevs = abbreviations if abbreviations is not None else DEFAULT_ABBREVIATIONS

    if merge_abbreviations:
        merged = []
        i = 0
        while i < len(tokens):
            tok = tokens[i]
...
class Window:
    index: int
    sentences: List[str]
    start_idx: int          # index of first sentence (inclusive)
    end_idx: int             # index of last sentence (exclusive)
...
def sliding_door(
    ...
    """
    Slide a window of `window_size` sentences across the document,
    advancing `step` sentences each time (step < window_size => overlap,
    which is what makes it a "sliding door" rather than disjoint chunking).

    A smaller step gives finer-grained localization of a style shift at
    the cost of more windows to score; a larger window gives each window
    more text to estimate stable stylometric features from, at the cost
    of blurring the exact boundary between two authors.

    Typical starting point for prose: window_size=15-30 sentences,
    step=window_size/4 to window_size/2.
    """
    n = len(sentences)
    windows = []
    i = 0
    ...
    while i < n:
        ...
        if drop_last_partial and len(chunk) < window_size and i != 0:
            break
        windows.append(Window(idx, chunk, i, i + len(chunk)))
        idx += 1
        if i + window_size >= n:
            break
        i += step
...
# ---------------------------------------------------------------------------
# 4. FEATURE HOOK (where the sliding door earns its keep)
# ---------------------------------------------------------------------------
...
def score_windows(
    ...
    """
    Apply a feature extractor to each window's text. Keep feature_fn
    focused on function-word frequencies, punctuation rates, average
    sentence/word length, POS-tag n-grams, etc. — the classic stylometric
    feature families — rather than content words, which track topic
    rather than authorship.
    """
    return [feature_fn(w.text) for w in windows]
